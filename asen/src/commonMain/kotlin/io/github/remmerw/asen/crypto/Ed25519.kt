...
internal object Ed25519 {
    ...
    private val GROUP_ORDER: ByteArray = byteArrayOf(
     ...
     * @param cached   cached projective point input
     */
    private fun sub(partialXYZT: PartialXYZT, extended: XYZT, cached: CachedXYT) {
        val t = LongArray(Field25519.LIMB_CNT)
     ...
     * Doubles `p` and puts the result into this PartialXYZT.
     */
    private fun doubleXYZT(partialXYZT: PartialXYZT, p: XYZT) {
     ...
     *
     *
     * Please note that this doesn't reuse since the below inputs are
     * byte values.
     */
     ...
     * @param pos in B[pos][] = (j+1)*B*256^pos
    ...
    private fun scalarMultWithBase(a: ByteArray): XYZ {
        ...
        for (i in 0 until Field25519.FIELD_LEN) {
            e[2 * i] = (((a[i].toInt() and 0xff)) and 0xf).toByte()
        ...
        for (i in 0 until e.size - 1) {
            e[i] = (e[i] + carry.toByte()).toByte()
            carry = e[i] + 8
            carry = carry shr 4
            e[i] = (e[i] - (carry shl 4).toByte()).toByte()
        }
        e[e.size - 1] = (e[e.size - 1] + carry.toByte()).toByte()
                ...
                while (b <= 6 && i + b < 256) {
                    if (r[i + b].toInt() != 0) {
                        if (r[i] + (r[i + b].toInt() shl b) <= 15) {
                            r[i] = (r[i] + (r[i + b].toInt() shl b).toByte()).toByte()
                            r[i + b] = 0
    ...
    private fun doubleScalarMultVarTime(a: ByteArray, pointA: XYZT, b: ByteArray): XYZ {
        ...
        while (i >= 0) {
            ...
            if (aSlide[i] > 0) {
                add(
                    t, XYZT.fromPartialXYZT(u, t),
                    pointAArray[aSlide[i] / 2]!!
                )
            } else if (aSlide[i] < 0) {
                sub(
                    t, XYZT.fromPartialXYZT(u, t),
            ...
            if (bSlide[i] > 0) {
                add(
                    t, XYZT.fromPartialXYZT(u, t),
                    Ed25519Constants.B2[bSlide[i] / 2]!!
                )
            } else if (bSlide[i] < 0) {
                sub(
                    t, XYZT.fromPartialXYZT(u, t),
    ...
    /**
     * Returns true if `in` is nonzero.
     *
     *
     * Note that execution time might depend on the input `in`.
     */
    private fun isNonZeroVarTime(input: LongArray): Boolean {
        val inCopy = LongArray(input.size + 1)
        input.copyInto(inCopy, 0, 0, input.size)
     ...
     */
    private fun getLsb(`in`: LongArray): Int {
        return Field25519.contract(`in`)[0].toInt() and 1
    }

    /**
     * Negates all values in `in` and store it in `out`.
     */
    private fun neg(out: LongArray, `in`: LongArray) {
        for (i in `in`.indices) {
            out[i] = -`in`[i]
        }
    }
     ...
     */
    private fun pow2252m3(out: LongArray, `in`: LongArray) {
        val t0 = LongArray(Field25519.LIMB_CNT)
    ...
    /**
     * Returns 3 bytes of `in` starting from `idx` in Little-Endian format.
     */
    private fun load3(`in`: ByteArray, idx: Int): Long {
        var result = `in`[idx].toLong() and 0xffL
        result = result or ((`in`[idx + 1].toInt() and 0xff).toLong() shl 8)
        result = result or ((`in`[idx + 2].toInt() and 0xff).toLong() shl 16)
        return result
    }

    /**
     * Returns 4 bytes of `in` starting from `idx` in Little-Endian format.
     */
    private fun load4(`in`: ByteArray, idx: Int): Long {
        var result = load3(`in`, idx)
        result = result or ((`in`[idx + 3].toInt() and 0xff).toLong() shl 24)
        return result
    }
    ...
    private fun reduce(s: ByteArray) {
        ...
        var carry6 = (s6 + (1 shl 20)) shr 21
        s7 += carry6
        s6 -= carry6 shl 21
        var carry8 = (s8 + (1 shl 20)) shr 21
        s9 += carry8
        s8 -= carry8 shl 21
        var carry10 = (s10 + (1 shl 20)) shr 21
        s11 += carry10
        s10 -= carry10 shl 21
        val carry12 = (s12 + (1 shl 20)) shr 21
        s13 += carry12
        s12 -= carry12 shl 21
        val carry14 = (s14 + (1 shl 20)) shr 21
        s15 += carry14
        s14 -= carry14 shl 21
        val carry16 = (s16 + (1 shl 20)) shr 21
        s17 += carry16
        s16 -= carry16 shl 21

        var carry7 = (s7 + (1 shl 20)) shr 21
        s8 += carry7
        s7 -= carry7 shl 21
        var carry9 = (s9 + (1 shl 20)) shr 21
        s10 += carry9
        s9 -= carry9 shl 21
        var carry11 = (s11 + (1 shl 20)) shr 21
        s12 += carry11
        s11 -= carry11 shl 21
        val carry13 = (s13 + (1 shl 20)) shr 21
        s14 += carry13
        s13 -= carry13 shl 21
        val carry15 = (s15 + (1 shl 20)) shr 21
        s16 += carry15
        s15 -= carry15 shl 21
        ...
        var carry0 = (s0 + (1 shl 20)) shr 21
        s1 += carry0
        s0 -= carry0 shl 21
        var carry2 = (s2 + (1 shl 20)) shr 21
        s3 += carry2
        s2 -= carry2 shl 21
        var carry4 = (s4 + (1 shl 20)) shr 21
        s5 += carry4
        s4 -= carry4 shl 21
        ...
        var carry1 = (s1 + (1 shl 20)) shr 21
        s2 += carry1
        s1 -= carry1 shl 21
        var carry3 = (s3 + (1 shl 20)) shr 21
        s4 += carry3
        s3 -= carry3 shl 21
        var carry5 = (s5 + (1 shl 20)) shr 21
        s6 += carry5
        s5 -= carry5 shl 21
     ...
     * where l = 2^252 + 27742317777372353535851937790883648493.
     */
    private fun mulAdd(s: ByteArray, a: ByteArray, b: ByteArray, c: ByteArray) {
        ...
        s17 += carry16
        s16 -= carry16 shl 21
        carry18 = (s18 + (1 shl 20)) shr 21
        s19 += carry18
        s18 -= carry18 shl 21
        carry20 = (s20 + (1 shl 20)) shr 21
        s21 += carry20
        s20 -= carry20 shl 21
        carry22 = (s22 + (1 shl 20)) shr 21
        s23 += carry22
        s22 -= carry22 shl 21
        ...
        s16 += carry15
        s15 -= carry15 shl 21
        carry17 = (s17 + (1 shl 20)) shr 21
        s18 += carry17
        s17 -= carry17 shl 21
        carry19 = (s19 + (1 shl 20)) shr 21
        s20 += carry19
        s19 -= carry19 shl 21
        carry21 = (s21 + (1 shl 20)) shr 21
        s22 += carry21
        s21 -= carry21 shl 21
     ...
     */
    fun sign(
        message: ByteArray, publicKey: ByteArray, hashedPrivateKey: ByteArray
    ): ByteArray {
        ...
        val messageCopy = message.copyOfRange(0, message.size)
        val digest = SHA512()
    ...
    fun init() {
        ...
        checkNotNull(Ed25519Constants.D) { "Could not initialize Ed25519." }
    }
    ...
    private class XYZ {
        ...
        val isOnCurve: Boolean
            ...
            get() {
                ...
                return equal(
                    Field25519.contract(lhs),
                    Field25519.contract(rhs)
                )
            }
        ...
        companion object {
             ...
             * ge_p1p1_to_p2.c
             */
            fun fromPartialXYZT(out: XYZ, `in`: PartialXYZT): XYZ {
    ...
    private class PartialXYZT {
        ...
        constructor(xyz: XYZ = XYZ(), t: LongArray = LongArray(Field25519.LIMB_CNT)) {
            this.xyz = xyz
            this.t = t
        }
    ...
    internal open class CachedXYT {
        ...
        open fun multByZ(output: LongArray, input: LongArray) {
            input.copyInto(output, 0, 0, Field25519.LIMB_CNT)
        }
     ...
     */(
        ...
        constructor(xyzt: XYZT) : this() {
            ...
            xyzt.xyz.z.copyInto(z, 0, 0, Field25519.LIMB_CNT)
            Field25519.mult(t2d, xyzt.t, Ed25519Constants.D2)
        }

        override fun multByZ(output: LongArray, input: LongArray) {
            Field25519.mult(output, input, z)
        }
    }
}
