...
internal object Field25519 {
     ...
     *
     */
    const val FIELD_LEN: Int = 32

    const val LIMB_CNT: Int = 10
     ...
     */
    fun sum(output: LongArray, `in`: LongArray) {
     ...
     *
     *
     * output must be distinct to both inputs. The inputs are reduced coefficient form, the output
     * is not.
     *
     *
     * out[] <= 14 * the largest product of the input limbs.
     */
    private fun product(out: LongArray, in2: LongArray, `in`: LongArray) {
        out[0] = in2[0] * `in`[0]
        ...
        out[5] =
            (in2[2] * `in`[3] + in2[3] * `in`[2] + in2[1] * `in`[4] + in2[4] * `in`[1] + in2[0] * `in`[5] + in2[5] * `in`[0])
        out[6] =
            (2 * (in2[3] * `in`[3] + in2[1] * `in`[5] + in2[5] * `in`[1]) + in2[2] * `in`[4] + in2[4] * `in`[2] + in2[0] * `in`[6] + in2[6] * `in`[0])
        out[7] =
            (in2[3] * `in`[4] + in2[4] * `in`[3] + in2[2] * `in`[5] + in2[5] * `in`[2] + in2[1] * `in`[6] + in2[6] * `in`[1] + in2[0] * `in`[7] + in2[7] * `in`[0])
        out[8] =
            (in2[4] * `in`[4] + 2 * (in2[3] * `in`[5] + in2[5] * `in`[3] + in2[1] * `in`[7] + in2[7] * `in`[1]) + in2[2] * `in`[6] + in2[6] * `in`[2] + in2[0] * `in`[8] + in2[8] * `in`[0])
        out[9] =
            (in2[4] * `in`[5] + in2[5] * `in`[4] + in2[3] * `in`[6] + in2[6] * `in`[3] + in2[2] * `in`[7] + in2[7] * `in`[2] + in2[1] * `in`[8] + in2[8] * `in`[1] + in2[0] * `in`[9] + in2[9] * `in`[0])
        out[10] =
            (2 * (in2[5] * `in`[5] + in2[3] * `in`[7] + in2[7] * `in`[3] + in2[1] * `in`[9] + in2[9] * `in`[1]) + in2[4] * `in`[6] + in2[6] * `in`[4] + in2[2] * `in`[8] + in2[8] * `in`[2])
        out[11] =
            (in2[5] * `in`[6] + in2[6] * `in`[5] + in2[4] * `in`[7] + in2[7] * `in`[4] + in2[3] * `in`[8] + in2[8] * `in`[3] + in2[2] * `in`[9] + in2[9] * `in`[2])
        out[12] =
            (in2[6] * `in`[6] + 2 * (in2[5] * `in`[7] + in2[7] * `in`[5] + in2[3] * `in`[9] + in2[9] * `in`[3]) + in2[4] * `in`[8] + in2[8] * `in`[4])
        out[13] =
            (in2[6] * `in`[7] + in2[7] * `in`[6] + in2[5] * `in`[8] + in2[8] * `in`[5] + in2[4] * `in`[9] + in2[9] * `in`[4])
        out[14] =
     ...
     * @param output An output array of size LIMB_CNT. After the call |output[]| < 2^26 will hold.
     */
    fun reduce(input: LongArray, output: LongArray) {
        ...
        if (input.size == 19) {
            ...
            tmp = LongArray(19)
            input.copyInto(tmp, 0, 0, input.size)
        }
        reduceSizeByModularReduction(tmp)
        reduceCoefficients(tmp)
        tmp.copyInto(output, 0, 0, LIMB_CNT)
    }
     ...
     *
     *
     * On entry: |output[]| < 14*2^54 On exit: |output[0..8]| < 280*2^54
     */
    private fun reduceSizeByModularReduction(output: LongArray) {
    ...
    /**
     * A helpful wrapper around : output = in * in2.
     *
     *
     * On entry: |in[]| < 2^27 and |in2[]| < 2^27.
     *
     *
     * The output is reduced degree (indeed, one need only provide storage for 10 limbs) and
     * |output[]| < 2^26.
     */
    fun mult(output: LongArray, `in`: LongArray, in2: LongArray) {
        val t = LongArray(19)
        product(t, `in`, in2)
        // |t[i]| < 2^26
        reduce(t, output)
     ...
     *
     *
     * output must be distinct from the input. The inputs are reduced coefficient form, the output
     * is not.
     *
     *
     * out[] <= 14 * the largest product of the input limbs.
     */
    private fun squareInner(out: LongArray, `in`: LongArray) {
        out[0] = `in`[0] * `in`[0]
        out[1] = 2 * `in`[0] * `in`[1]
        ...
        out[9] =
            2 * (`in`[4] * `in`[5] + `in`[3] * `in`[6] + `in`[2] * `in`[7] + `in`[1] * `in`[8] + `in`[0] * `in`[9])
        out[10] =
            2 * (`in`[5] * `in`[5] + `in`[4] * `in`[6] + `in`[2] * `in`[8] + 2 * (`in`[3] * `in`[7] + `in`[1] * `in`[9]))
        out[11] =
            2 * (`in`[5] * `in`[6] + `in`[4] * `in`[7] + `in`[3] * `in`[8] + `in`[2] * `in`[9])
        out[12] =
            `in`[6] * `in`[6] + 2 * (`in`[4] * `in`[8] + 2 * (`in`[5] * `in`[7] + `in`[3] * `in`[9]))
     ...
     *
     *
     * On entry: The |in| argument is in reduced coefficients form and |in[]| < 2^27.
     *
     *
     * On exit: The |output| argument is in reduced coefficients form (indeed, one need only
     * provide storage for 10 limbs) and |out[]| < 2^26.
     */
    fun square(output: LongArray, `in`: LongArray) {
        val t = LongArray(19)
        squareInner(t, `in`)
    ...
    fun contract(inputLimbs: LongArray): ByteArray {
        ...
        repeat(1) {
            for (i in 0..8) {
                ...
                input[i] = input[i] + (carry.toLong() shl SHIFT[i and 1])
        ...
        for (i in 0 until LIMB_CNT) {
            output[EXPAND_START[i]] =
                (output[EXPAND_START[i]]
                    .toInt() or (input[i] and 0xffL).toByte()
                    .toInt()).toByte()
            output[EXPAND_START[i] + 1] = (output[EXPAND_START[i] + 1]
                .toInt() or ((input[i] shr 8) and 0xffL).toByte().toInt()).toByte()
            output[EXPAND_START[i] + 2] = (output[EXPAND_START[i] + 2]
                .toInt() or ((input[i] shr 16) and 0xffL).toByte().toInt()).toByte()
            output[EXPAND_START[i] + 3] = (output[EXPAND_START[i] + 3]
                .toInt() or ((input[i] shr 24) and 0xffL).toByte().toInt()).toByte()
        }
        return output
    }
